...
能保证升级后你的应用还可以正常工作的万全之策在升级之前拥有良好的测试覆盖。如果你还没有对你的应用进行过大量的自动化测试，你需要花时间手动对所有做过改动的部分进行测试。 对于 Rails 升级而言，这意味着应用中的每一个 functionality 。想对自己好点儿的话在升级 Rails 之前还是确认 test coverage 正确无误吧。
